...
function analyzeQuotes(quotes) {
  ...
  const bestIdx = parsed.reduce((best, _, i) => {
    ...
    return Math.abs(parsed[i + 1].close - parsed[i].close) >
      Math.abs(parsed[best + 1].close - parsed[best].close)
      ? i
      : best;
  }, 0);
  ...
  return {
    biggestSameDayDiff: { ...biggestSameDayDiff, difference: percentageDiff(biggestSameDayDiff.open, biggestSameDayDiff.close) },
...
export function useStock(ticker, { compareTicker, period } = {}) {
  ...
  const periodAnalysis = useMemo(
    () => analyzeQuotes(filterByPeriod(allQuotes, period)),
    [allQuotes, period],
  );
