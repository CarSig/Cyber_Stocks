import type { CorrelationResult, SentimentLabel } from '@/types';

type SentimentCorrelationMetricsProps = {
  sentimentInfo?: SentimentLabel | null;
  hasCorrelation?: boolean;
  strength?: SentimentLabel | null;
  corrResult?: CorrelationResult | { error: string };
};

export default function SentimentCorrelationMetrics({
  sentimentInfo,
  hasCorrelation,
  strength,
  corrResult,
}: SentimentCorrelationMetricsProps) {
  return (
    <div className="card-metrics">
      {sentimentInfo && (
        <div>
          <div className="card-metric-label">Sentiment</div>
          <span className="card-metric-value" style={{ color: sentimentInfo.color }}>
            {sentimentInfo.label}
          </span>
        </div>
      )}
      {hasCorrelation && strength && corrResult && !('error' in corrResult) && (
        <div>
          <div className="card-metric-label">Correlation</div>
          <span className="card-metric-value" style={{ color: strength.color }}>
            {strength.label}
          </span>
          <div className="card-metric-details">
            <span>
              r: <strong>{(corrResult as CorrelationResult).r.toFixed(3)}</strong>
            </span>
            <span>
              p:{' '}
              <strong>
                {(corrResult as CorrelationResult).pValue < 0.001
                  ? '< 0.001'
                  : (corrResult as CorrelationResult).pValue.toFixed(3)}
              </strong>
            </span>
          </div>
        </div>
      )}
    </div>
  );
}
