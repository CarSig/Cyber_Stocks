...
export default function CorrelationMatrix() {
  ...
  function applyCustomDate() {
    if (!draftStart && !draftEnd) { setDateError("Set at least one date."); return; }
    if (draftStart && draftEnd && draftStart >= draftEnd) { setDateError("Start must be before end."); return; }
      ...
      <div className="corr-matrix-wrap">
        <div className="corr-matrix-toolbar" style={{ flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {WINDOW_OPTS.map(({ label, value }) => (
              <button
                key={value}
                className={`corr-lag-btn${!isCustom && windowDays === value ? " corr-lag-btn--active" : ""}`}
                onClick={() => { setWindowDays(value); setStartDate(""); setEndDate(""); }}
              >
                {label}
              </button>
            ))}
            <button
              className={`corr-lag-btn${isCustom ? " corr-lag-btn--active" : ""}`}
              onClick={openModal}
            >
              {customLabel}
            </button>
        ...
        {data && (() => {
          const thinTickers = new Set(
            data.names.filter((a) =>
              data.names.every((b) => a === b || data.matrix[a]?.[b] == null)
            )
          );
        ...
        <DialogContent style={{ maxWidth: 320 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, paddingTop: 4 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label className="corr-matrix-label" style={{ fontSize: 12 }}>Start date</label>
              <input
                type="date"
                value={draftStart}
                min={DATA_START}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label className="corr-matrix-label" style={{ fontSize: 12 }}>End date</label>
              <input
                type="date"
                value={draftEnd}
                min={draftStart || DATA_START}
                max={today}
            ...
            {dateError && <p className="corr-lag-note" style={{ color: "var(--color-amber)", margin: 0 }}>{dateError}</p>}
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              {isCustom && (
                <button className="corr-lag-btn" onClick={clearCustomDate}>Clear</button>
              )}
              <button
