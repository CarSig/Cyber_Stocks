...
export const NVD_SEVERITY_COLORS = {
  CRITICAL: '#ef4444',
  HIGH: '#f97316',
  MEDIUM: '#eab308',
  LOW: '#60a5fa',
...
export function buildMarkers(analysis, color) {
  ...
  return [
    same?.date && {
      time: new Date(same.date).toISOString().slice(0, 10),
      position: 'aboveBar',
      color: amber,
      shape: 'circle',
    ...
    next?.[0]?.date && {
      time: new Date(next[0].date).toISOString().slice(0, 10),
      position: 'belowBar',
      color: red,
      shape: 'arrowUp',
      text: 'Move D1',
    ...
    next?.[1]?.date && {
      time: new Date(next[1].date).toISOString().slice(0, 10),
      position: 'belowBar',
      color: red,
      shape: 'arrowUp',
      text: 'Move D2',
  ...
  return [...byTrading.entries()]
    .flatMap(([time, { pos, neg, neu, snapped }]) => {
      ...
      return [
        ...
        ...(neu
          ? [{ time, position: neg ? 'aboveBar' : 'belowBar', color: '#eab308', shape: 'circle', text: `${neu}${s}` }]
          : []),
      ];
    })
    .sort((a, b) => a.time.localeCompare(b.time));
...
export function buildOverlayMarkers(overlayRefs, quotes, analysis, showAnalysis) {
  ...
  return [
    ...
    ...(showOtxRef.current
      ? buildCountMarkers(otxPulsesRef.current, {
          dateField: 'created',
          position: 'belowBar',
          color: '#a855f7',
          shape: 'circle',
      ...
      : []),
    ...(showKevRef.current
      ? buildCountMarkers(kevItemsRef.current, {
          dateField: 'dateAdded',
          position: 'belowBar',
          color: '#f97316',
          shape: 'arrowDown',
