...
export type SimStrategy<A> = {
  ...
  createAction: (id: number, label: string, side: string, rawValue: string) => A | null;
...
export const intradayStrategy: SimStrategy<Action> = {
  ...
  createAction: (id, time, side, rawValue) => {
    const val = Number(rawValue);
    if (!isFinite(val) || val <= 0) return null;
    const isExit = side === 'sell' || side === 'cover';
    return {
      id,
      timestamp: DateUtils.timeToIso(time, ''),
      time,
      side: side as Side,
...
type LongTermAction = { id: number; date: string; type: 'buy' | 'sell'; value: string };

export const longTermStrategy: SimStrategy<LongTermAction> = {
  toText: (_date, actions) =>
    actions
      .map((a) => `${a.date}, ${a.type === 'sell' ? -Math.abs(Number(a.value)) : Math.abs(Number(a.value))}`)
      .join('\n'),
