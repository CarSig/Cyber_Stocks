...
function useAlpacaBars(ticker: string, date: string) {
  return useQuery<AlpacaBarsResponse>({
...
function IntradayChart({ bars }: IntradayChartProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const chartRef = useRef<ReturnType<typeof createChart> | null>(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- lightweight-charts series type varies by series constructor
  ...
  useEffect(() => {
    ...
    const data = bars.map((b) => ({
      time: Math.floor(new Date(b.t).getTime() / 1000) as unknown as `${number}-${number}-${number}`,
      open: b.o,
      high: b.h,
      low: b.l,
      close: b.c,
