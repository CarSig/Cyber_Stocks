...
export default function TickerWatchlist({ ticker, companies, sparklinesData, showWatchlist, onToggle }) {
          ...
          {sortedCompanies.map(([name, t]) => {
            ...
            return (
              <Link key={t} to={`/${t}`} className={`ticker-wl-row${t === ticker ? ' is-active' : ''}`}>
                ...
                {sp?.closes?.length > 1 && <WatchlistSparkline closes={sp.closes} color={sparkColor} id={t} />}
              </Link>
            );
          })}
        </div>
