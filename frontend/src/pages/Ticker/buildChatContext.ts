// eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic financial summary object
...
function corrLine(
  ...
  if (lagImpact?.avgReturn != null)
    parts.push(`avg return after event: ${pct(lagImpact.avgReturn)} over ${lagImpact.lagDays ?? '?'}d`);
  return parts.length ? `${label}: ${parts.join(', ')}` : null;
}

type BuildChatContextOpts = {
  ticker: string;
  summary?: AnyRecord;
  periodAnalysis?: AnyRecord;
  newsArticles?: AnyRecord[];
  newsAnalysisData?: AnyRecord;
  simulationResult?: AnyRecord;
  researchSections?: Array<{ title: string; text: string }>;
  trump?: AnyRecord;
  nvdData?: AnyRecord;
  otxData?: AnyRecord;
  kevData?: AnyRecord;
  threatIntel?: AnyRecord;
  correlationData?: AnyRecord;
  compareTicker?: string;
  intelligenceArticles?: AnyRecord[];
  allQuotes?: Array<{ date: string; close: number; volume?: number }>;
};
...
export function buildChatContext({
  ...
  lines.push(`\n--- Price ---`);
  lines.push(
    `Current: $${fd?.currentPrice?.toFixed(2) ?? last?.close?.toFixed(2) ?? 'N/A'} | Day change: ${dayChange}`,
  );
  lines.push(
    `52w High: $${sd?.fiftyTwoWeekHigh?.toFixed(2) ?? 'N/A'} | 52w Low: $${sd?.fiftyTwoWeekLow?.toFixed(2) ?? 'N/A'}`,
  );
  lines.push(
    `50d MA: $${sd?.fiftyDayAverage?.toFixed(2) ?? 'N/A'} | 200d MA: $${sd?.twoHundredDayAverage?.toFixed(2) ?? 'N/A'}`,
  );
  ...
  lines.push(
    `P/E (trailing): ${sd?.trailingPE?.toFixed(2) ?? 'N/A'} | P/E (forward): ${sd?.forwardPE?.toFixed(2) ?? 'N/A'}`,
  );
  lines.push(`PEG: ${ks?.pegRatio?.toFixed(2) ?? 'N/A'} | P/B: ${ks?.priceToBook?.toFixed(2) ?? 'N/A'}`);
  lines.push(
    `EV/Revenue: ${ks?.enterpriseToRevenue?.toFixed(2) ?? 'N/A'} | EV/EBITDA: ${ks?.enterpriseToEbitda?.toFixed(2) ?? 'N/A'}`,
  );
  ...
  lines.push(`\n--- Financials ---`);
  lines.push(
    `Revenue: ${fmt(fd?.totalRevenue, '$')} | Revenue growth: ${fd?.revenueGrowth != null ? pct(fd.revenueGrowth) : 'N/A'}`,
  );
  lines.push(
    `Gross profit: ${fmt(fd?.grossProfits, '$')} | Gross margin: ${fd?.grossMargins != null ? pct(fd.grossMargins) : 'N/A'}`,
  );
  lines.push(
    `Operating margin: ${fd?.operatingMargins != null ? pct(fd.operatingMargins) : 'N/A'} | Profit margin: ${fd?.profitMargins != null ? pct(fd.profitMargins) : 'N/A'}`,
  );
  lines.push(
    `EBITDA: ${fmt(fd?.ebitda, '$')} | Net income: ${inc?.netIncome != null ? fmt(inc.netIncome, '$') : 'N/A'}`,
  );
  lines.push(`Free cash flow: ${fmt(fd?.freeCashflow, '$')} | Operating cash flow: ${fmt(fd?.operatingCashflow, '$')}`);
  lines.push(
    `Total cash: ${fmt(fd?.totalCash, '$')} | Total debt: ${fmt(fd?.totalDebt, '$')} | D/E ratio: ${fd?.debtToEquity?.toFixed(2) ?? 'N/A'}`,
  );
  lines.push(
    `Return on equity: ${fd?.returnOnEquity != null ? pct(fd.returnOnEquity) : 'N/A'} | Return on assets: ${fd?.returnOnAssets != null ? pct(fd.returnOnAssets) : 'N/A'}`,
  );
  if (earn?.earningsChart?.quarterly?.length) {
    const q = earn.earningsChart.quarterly.slice(-4);
    lines.push(
      `EPS (last 4 qtrs): ${q.map((e: AnyRecord) => `${e.date} A:${e.actual?.fmt ?? '?'} E:${e.estimate?.fmt ?? '?'}`).join(' | ')}`,
    );
  }
  ...
  if (newsArticles?.length) {
    ...
    const withSentiment = newsAnalysisData
      ? newsArticles
    ...
    if (top.length && newsAnalysisData)
      lines.push(
        `Most positive: ${top.map((a) => `"${a.title}" (${newsAnalysisData[a.link].sentiment.toFixed(2)})`).join(' | ')}`,
      );
    if (bottom.length && newsAnalysisData)
      lines.push(
        `Most negative: ${bottom.map((a) => `"${a.title}" (${newsAnalysisData[a.link].sentiment.toFixed(2)})`).join(' | ')}`,
      );
    lines.push(
      `Recent: ${newsArticles
        .slice(0, 5)
  ...
  if (intelligenceArticles?.length) {
    ...
    lines.push(
      intelligenceArticles
        .slice(0, 5)
        .map((a) => `${a.timestamp?.slice(0, 10) ?? '?'}: ${a.id}`)
        .join('\n'),
    );
  }
  ...
  if (trump?.posts?.length) {
    ...
    trump.posts.slice(0, 5).forEach((p: AnyRecord) => {
  ...
  if (nvdItems.length) {
    ...
    nvdItems
      ...
      .forEach((v: AnyRecord) =>
        lines.push(
          `${v.published?.slice(0, 10) ?? '?'} [${v.severity ?? '?'}] ${v.cveId ?? v.cveID}: ${v.description?.slice(0, 100)}`,
        ),
      );
  ...
  if (otxItems.length) {
    ...
    otxItems.slice(0, 5).forEach((p: AnyRecord) => lines.push(`${p.created?.slice(0, 10) ?? '?'}: ${p.name}`));
  ...
  if (simulationResult) {
    ...
    lines.push(
      `Shares held: ${Number(simulationResult.finalShares).toFixed(4)} | Current value: $${Number(simulationResult.sharesValue).toFixed(2)}`,
    );
    lines.push(`Cash withdrawn: $${Number(simulationResult.cashWithdrawn).toFixed(2)}`);
    lines.push(
      `Transactions: ${simulationResult.transactions.map((t: AnyRecord) => `${t.date} ${t.type} ${Number(t.shares).toFixed(4)}sh @ $${Number(t.price).toFixed(2)}`).join(', ')}`,
    );
  }
