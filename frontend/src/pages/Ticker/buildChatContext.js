...
export function buildChatContext({
  ...
  if (earn?.earningsChart?.quarterly?.length) {
    ...
    lines.push(
      `EPS (last 4 qtrs): ${q.map((e) => `${e.date} A:${e.actual?.fmt ?? '?'} E:${e.estimate?.fmt ?? '?'}`).join(' | ')}`,
    );
  }
  ...
  if (simulationResult) {
    ...
    lines.push(
      `Transactions: ${simulationResult.transactions.map((t) => `${t.date} ${t.type} ${Number(t.shares).toFixed(4)}sh @ $${Number(t.price).toFixed(2)}`).join(', ')}`,
    );
  }
