...
function IntradayChart({ bars, compareBars, timezone, chartType }) {
  ...
  // Re-draw all series whenever bars, compare, timezone or chart type changes
  useEffect(() => {
    const chart = chartRef.current;
    if (!chart || !bars?.length) return;

    // Remove all existing series by replacing them
    chart.applyOptions({});
    ...
    chart._seriesRefs.push(mainSeries);

    // Compare series — always line, on separate price scales
    compareBars.forEach(({ ticker: cTicker, bars: cBars }, i) => {
      ...
      cSeries.setData(cBars.map((b) => ({ time: toTzTime(b.t, cTzOffset), value: b.c })));
      cSeries.applyOptions({ title: cTicker });
      chart._seriesRefs.push(cSeries);
    });
...
// Fetches bars for a single compare ticker — used as a child query
function CompareQuery({ ticker, date, timeframe, onData }) {
