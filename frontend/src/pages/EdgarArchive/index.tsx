...
// Chart-heavy per-company view: only rendered after a ticker is selected, so it
// (and the whole charts framework it pulls in) is split out of the initial chunk.
const SecArchiveContent = lazy(() => import('./SecArchiveContent'));
...
function PerCompanyTab() {
  ...
  if (selectedTicker) {
    return (
      <Suspense fallback={<p className="edgar-status">Loading…</p>}>
        <SecArchiveContent key={selectedTicker} ticker={selectedTicker} onBack={() => setSelectedTicker(null)} />
      </Suspense>
    );
  }
