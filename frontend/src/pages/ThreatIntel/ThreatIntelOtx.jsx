...
export default function ThreatIntelOtx() {
  ...
  const { page, setPage, data, isPending, error, totalPages, companiesData } = usePaginatedThreatIntel("otx", { company });

  const withReset = useFilterWithPageReset(setPage);

  return (
    <ThreatIntelShell title="AlienVault OTX" isPending={isPending} error={error} data={data} page={page} setPage={setPage} totalPages={totalPages}>
