...
const EXIT_DESCRIPTIONS: Record<ExitStrategy, { label: string; description: string }> = {
  ...
  '15:59': {
    label: 'Exit 15:59',
    description:
      'Fixed exit at the last bar of the session (16:00 minus one bar width). Captures the full trading day.',
  },
  'vol-same-day': {
    label: 'Vol same day (ATR)',
    description:
      "Exits on the first bar where |close − entry| ≥ the entry bar's high-low range (ATR proxy). Falls back to 15:45 if the target is never hit intraday.",
  },
  'vol-next-day': {
    label: 'Vol next day (2% move)',
    description:
      "Exits on the first bar where price moves ≥ 2% from entry, scanning same day through close of next trading day. Falls back to next day's last bar.",
  },
  'vol-hold': {
    label: 'Vol hold 2× spike',
    description:
      "Holds until a bar's volume reaches 2× the average volume of all pre-entry bars (climactic move). Scans up to 5 trading days. Falls back to the last available bar.",
  },
  'vol-hold-3x': {
    label: 'Vol hold 3× spike',
    description:
      'Stricter version of vol-hold — only exits on a 3× average-volume bar. Filters out minor spikes and waits for a decisive capitulation or euphoria bar.',
  },
  'vol-hold-eod': {
    label: 'Vol hold EOD spike',
    description:
      "Checks volume only on the closing bar of each day. Exits when the day's closing bar volume exceeds 2× average. Ignores intrabar noise.",
  },
  'vol-hold-vwap': {
    label: 'Vol hold VWAP cross',
    description:
      'Exits when price crosses VWAP. On a long trade, exits when close drops below VWAP (distribution signal). On a short, exits when close rises above VWAP. Scans up to 5 days.',
  },
  'vol-hold-confirm': {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[calc(48rem+250px)]" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        <div
          style={{ display: 'flex', gap: '0.75rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--border)' }}
        >
