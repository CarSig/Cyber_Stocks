type SentimentStyle = { color: string; icon: string };

export function sentimentScoreStyle(score: number | null | undefined): SentimentStyle {
...
export function sentimentToColor(score: number): string {
  ...
  const [from, to, t] = clamped < 0 ? [RED, AMBER, clamped + 1] : [AMBER, GREEN, clamped];
